// src/pages/Dashboard/IntakeLabels.tsx
...
export default function IntakeLabels() {
  const [labels, setLabels] = useState<IntakeLabel[]>([]);
  ...
  const [modalTitle, setModalTitle] = useState("Add Intake Label");
  ...
  const fetchLabels = async () => {
    try {
      setLoading(true);
      const res = await api.get("/intake-labels");
      setLabels(res.data.data || res.data || []);
    } catch {
      toast.error("Failed to fetch intake labels");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openAddModal = () => {
    setForm({ name: "" });
    setFormError("");
    setModalTitle("Add Intake Label");
    setIsModalOpen(true);
  };

  const openEditModal = (label: IntakeLabel) => {
    setForm({ id: label.id, name: label.name });
    setFormError("");
    setModalTitle("Edit Intake Label");
    setIsModalOpen(true);
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (form.id) {
        await api.put(`/intake-labels/${form.id}`, { name: form.name });
        toast.success("Intake label updated successfully!");
      } else {
        await api.post("/intake-labels", { name: form.name });
        toast.success("Intake label created successfully!");
      }
      setIsModalOpen(false);
      fetchLabels();
    } catch {
      toast.error("Error saving intake label");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await api.delete(`/intake-labels/${deleteTargetId}`);
      toast.success("Intake label deleted successfully");
      fetchLabels();
    } catch {
      toast.error("Error deleting intake label");
    } finally {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-200 dark:border-gray-700">
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Are you sure you want to delete this intake label?{" "}
